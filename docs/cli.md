...
所有子命令读写同一个数据目录：默认为用户主目录下的 `~/.missioncrew/`，可用环境变量 `MISSIONCREW_HOME` 覆盖。数据目录不存在时会静默新建；若服务实例用的是别的目录（例如 pm2 部署默认使用仓库内的 `.missioncrew/`），执行 CLI 前要设置同一个 `MISSIONCREW_HOME`，否则操作的是一份空数据库。`mc --help` 与各子命令的 `--help` 列出完整参数，本文按用途归纳。
