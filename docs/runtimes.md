...
`RuntimeManager.account_usage()` 并行探测支持该能力的 Runtime，并在服务内按 backend 缓存 60 秒；同一 Runtime 的探测进行中时，并发到达的请求（重复进入页面、多端同时打开、任务结束联动）共用这一次结果，不会再拉起第二个进程互相拖慢、再按完成先后覆盖缓存。账户用量在进入运行状态页或点击“立即刷新”时读取；存在至少一个已开启用量联动的项目角色时，Runtime 任务结束也会触发一次读取。页面停留期间的 10 秒常规轮询只更新 Runtime 状态和调用历史，不请求账户用量，也没有周期性账户探测线程。任务结束事件只唤醒一次刷新，短时间内连续完成的事件会合并；没有联动角色时直接忽略该事件。探测失败只让对应卡片显示“需要登录”或“暂不可用”，不会影响运行状态、聊天执行或其他 Runtime 的限额。限额快照只保存在内存，不写入 SQLite；API 不返回 access token、refresh token、用户标识、凭据路径或上游错误正文。
...
| Antigravity | 执行 `agy -p "/usage" --output-format json`，解析 `command.data.groups[].buckets[]`，不启动推理或创建 conversation。CLI 每次都先启动语言服务器、静默登录并拉取实验配置，正常也要 4～9 秒才执行到 `/usage`，因此超时或非登录类失败会换新进程自动重试一次 | CLI 自己读取和刷新当前登录态；MissionCrew 不读取凭据 | `remaining_fraction` 换算为已用百分比，`reset_time` 映射重置时间，`window=weekly` 映射 7 天窗口。Gemini 与 Claude/GPT 分池显示。角色联动仅对明确绑定 Gemini / Claude / GPT 型号的角色应用对应的 `gemini-weekly` / `3p-weekly` 桶；默认或未知模型不猜所属池。仍有正余额时不会因舍入显示为 100% 而误停角色。协议入口见 [Antigravity /usage 文档](https://antigravity.google/docs/cli/commands/usage/) |
...
后台命令(Bash `run_in_background`,即 `task_type=local_bash` 等非 `_agent` 任务)的语义与后台 Agent 相反:它们不阻塞顶层 `result`,chat run 正常结束,后续消息照常处理;任务清单跨 turn 存活并计入实例快照,状态页在实例行显示「N 个后台命令」,提示回收、停止或重启该实例会连带终止这些命令。任务结束时 Claude CLI 会在会话空闲处自发开启一个无用户输入的汇报 turn;唤醒经 `RuntimeManager.set_wake_handler(handler, begin=…)` 注册的两段回调交给 ChatEngine:provider 在 turn 起点(空闲时收到 `init`/`assistant`/`stream_event`)**同步**调用 `begin`(session_key、角色、触发任务),ChatEngine 立刻以一条平台消息(「后台命令已结束」)为触发锚点落成运行、签发绑定该运行的 Agent Tool 令牌并返回实时事件接收器,turn 内的过程事件直达运行(begin 期间已缓冲的先按序补发);turn 的 `result` 到达后再把最终输出连同 `run_id` 交给 `handler` 收尾(发布汇报或记失败、撤销令牌)。必须在 turn 起点签发令牌:常规运行结束时 `deactivate_run_token` 已撤销并删除稳定路径上的令牌文件,而唤醒 turn 在平台知晓之前就在 CLI 进程里执行,若等 turn 结束再落成运行,turn 内的平台写操作(发布文档、更新 Task)会全部因缺少令牌失败。`begin` 缺席、返回 None(会话不存在、频道停止中、令牌签发失败)或抛异常时 provider 退回旧路径:事件缓冲,turn 结束后整体交给 `handler` 新建运行并回放。已落成运行的唤醒 turn 无论有无输出都会收尾;进程中途被关(停止、重启)时 `close()` 以失败 payload 通知平台,运行不会悬在 running。显式频道 run 在发送用户输入前会等待正在进行的 wake turn 完成，并以同一条件锁登记活动 sink，防止 wake sink 吞掉新 run 的事件或最终 `result`。派发语义继承自**启动后台任务的那轮**:任务注册时记下该轮的触发消息(`ExecutionConfig.trigger_message_id`),唤醒时如果那条消息是人类直接点名该角色,汇报挂回原人类消息且不交回主控(与直接点名的常规回复一致);否则非主控角色的结果自动交回主控验收。会话记录不存在、频道已归档或角色停用时唤醒被静默丢弃。CLI 把每条 `task_notification` 排成队列,turn 之间按先后**各起一个**自唤醒 turn(两条通知同时到达也是两个 turn,2.1.261 实测);turn 进行中到达且该 turn 随后还有模型调用的通知则随那次调用就地喂给模型,不再单独唤醒。provider 按同样规则维护待唤醒队列:通知到达即入队并记下所在 turn,该 turn 再收到 `user` 消息(又要调模型)就剔除本 turn 内入队的条目,唤醒 turn 开始时只取队首一条作为它的任务——这样同批的第二个唤醒仍带着自己的 `origin_trigger`,不会因为第一个唤醒把原因整体取走而退化成"来源不明、交回主控"。自唤醒 turn 没有对应运行,工具审批沿用该会话最近一轮的策略:`auto` 放行(审批事件写进唤醒缓冲随汇报回放),`prompt` 因无可交互的运行而明确拒绝,提问工具同样拒绝;否则唤醒里的 Agent 连后台输出都读不到,只能发一条空汇报。迟到的裸 `result`(无 init/assistant 前导)不会被包装成唤醒汇报。进程带着未结束的后台命令被杀(停止、重启服务)后,下次 `--resume` 启动时 CLI 会在首个 `init` 之前自行补一条该任务的 `task_notification`(status=stopped,"No completion record was found…"),并紧跟一对不调模型的空 `init`/`result`(`num_turns=0`),之后才处理 stdin 里的用户消息(2.1.258 实测,与消息何时写入无关);provider 把首个 init 前到达的通知计为回放,随后 `num_turns=0` 的成功 result 不作为本轮结束(本轮尚无模型活动时同样跳过;中断后的 result 一律认),两者都以 status 事件写进过程流。否则本轮会被判成"正常退出无输出",而模型随后的真实工作会落在运行之外:工具权限被拒、最终文本被当成自唤醒汇报开成新根帖。远程长任务(如 ssh 部署)可用后台 ssh 保持跟踪,ssh 退出即触发自动汇报。
...
Codex 的 workspace-write 沙箱还会把每个可写根下的 `.git`、`.agents`、`.codex` 以及 worktree 解析出的主仓 `.git/worktrees/<名字>` 挂成只读，git 写操作与仓内 Skill 编辑在沙箱内会报 `Read-only file system`。平台不改写这些根：把 `.git` 的子目录列为额外可写根会让沙箱因建占位挂载点失败而起不来，整个 `.git` 放开则暴露 hooks/config。取而代之的是公共上下文「必须遵守」与 manual 第 3 节引导 Agent 对清单内路径的这类失败申请升级重跑，而不是判定环境只读后放弃。
...
上面的续接机制服务"短后台任务在 turn 内收尾";长后台任务走客户端终端 + 自唤醒。MissionCrew 在 `initialize` 声明 `clientCapabilities.terminal=true` 并实现 `terminal/create|output|wait_for_exit|kill|release`(`_ClientTerminal`):agent 把命令交给客户端执行,**进程归 MissionCrew 持有**,退出时间因此对平台可见;输出超限时按 ACP 约定从头部截断。实测 Grok 会据此把 `background=true` 的命令放进客户端终端并立即结束 turn(阻塞式 `wait_for_exit` 由客户端在进程退出后异步应答,不占协议读取线程),终端退出后 Grok 通过自有扩展(`_x.ai/task_completed` + 注入合成 user message)在 turn 之外自发跑一轮汇报。平台把 turn 之外到达的 `session/update` 缓冲进唤醒管线:按静默间隔(3s)判定自发 turn 结束后,连同触发任务信息经 `RuntimeManager.set_wake_handler` 交给 ChatEngine 落成频道里的新运行(与 Claude 后台命令的唤醒同一条管线,但 ACP 没有可靠的 turn 起点,仍走 turn 结束后整体交付,自发汇报 turn 内没有 Agent Tool 令牌);新运行 turn 开始时会先交付未派发的缓冲,自发汇报不会混入其他运行。kimi/trae 二进制同样包含 terminal/* 实现,行为待配额恢复后验证。带存活客户端终端的长驻会话不参与空闲回收(关闭客户端会连带杀掉终端进程),实例快照以 `background_tasks` 上报终端数;服务退出仍会终止全部终端,与 Claude 后台命令口径一致。
...
聊天 Prompt 分为两部分：MissionCrew 公共上下文和本轮任务输入。公共上下文按“先读的先给”排列：身份行（角色、频道及其 id、项目与描述、工作目录绝对路径、代码仓）→「怎么干活」（执行角色与主控各一套五步流程，规则挂在步骤上）→ Agent Tool 调用方式与可用动作 →「必须遵守」（可读写目录、边界、路径失败处理、引用规则、Task 快照只读、工作区不进业务仓、Skill 来源）→「项目资料」（章程、开发准则、准则与 Skill 索引）→「工作区一览」（根目录一次、子项相对名与环境变量、手册指引）→ 主控专属的「项目主控职责」（纪律、按需查询、协作链预算）与「项目清单」。本轮任务输入以「本轮触发」标明触发者类型（人类消息、执行角色回报、主控派发、自动化），再给触发消息 JSON。公共上下文明确 MissionCrew 是 Agent harness 而不是业务代码仓，并说明 harness 文件不会进入业务源码。公共区块带内容哈希版本及压缩提示，要求 Runtime 只压缩普通对话、工具过程和任务细节，完整保留最新公共区块。项目或角色设置变更会改变版本；准则与 Skill 的正文不进入 Prompt，也不参与公共上下文哈希，正文修改只在下一轮的本轮输入里附一段「资源更新」提示（列出被修改的准则/Skill，要求重新读取），不触发完整重发；name/description 等索引信息变化仍会改变版本。后收到的版本整体替换旧版本。公共上下文按渐进式披露组织：Prompt 只保留必须时刻遵守的规则，平台各动作的用法、面板/数据源/准则/Skill/自动化的格式约定和文件系统边界细节都写在工作区的 `manual.md`（环境变量 `MISSIONCREW_MANUAL`，每轮重新渲染），由 Agent 按需读取。主控看到的角色名册、代码仓、面板、数据源、自动化清单作为「项目清单」附在公共区块末尾但不参与哈希，频道只列活跃 id、详情与归档频道走 `channel.list` 动作按需查询；会话保存上次看到的条目版本，条目新增、更新、归档或停用时只在下一轮的本轮输入里列出差异，不触发完整重发。
...
Agent Tool 公共区块列出当前角色的动作 scope，并注入 `MISSIONCREW_AGENT_TOOL_URL`、`MISSIONCREW_AGENT_TOKEN_FILE` 和 `MISSIONCREW_AGENT_TOOL_PYTHON`。同一 `channel × role` 的 Run 串行获得执行锁；平台随后在稳定令牌路径原子写入绑定该 Run 的 capability，CLI 不提交 `run_id`，API 从已认证 token 确定归属。Run 结束即撤销令牌并删除该文件；Claude 后台命令的自唤醒汇报 turn 在起点经 `begin` 回调落成运行并重新签发令牌，turn 内的写操作与常规 Run 同等鉴权。工具的结构化错误可以在当前 Agent 回合内处理，而最终回复文本块只能在回合结束后解析，因此历史文本块只保留兼容读取。
