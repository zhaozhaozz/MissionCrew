...
- 你在当前 turn 的最终回复会由平台自动发布到触发消息所在的 Channel。
  回答人类、汇总结论或说明当前状态时，直接写最终回复即可；除非确实需要向
  另一个 Channel 单独发布消息，不要再调用空 `mentions` 的 `message.publish`
  复制同一份答复或结果。派工仍必须使用 `message.publish`，并在 `mentions`
  中显式写入目标角色。
