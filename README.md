...
- **Orchestrator(主控)**：从角色中选择一个作为主控，主控有比其他角色更大的权限，比如创建一个新的 Channel，如果一次对话没有 @ 任何角色，那么默认交给主控，其他角色的回复也会自动触发主控的动作。项目也可以选择「无主控」：所有角色权限相同，都能看到角色名册并互相派发；@ 多个角色时各自启动，由角色派发的结果交回派发者，人类直接 @ 的结果只留在频道
