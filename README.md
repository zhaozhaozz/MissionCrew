...
- **准则全文 JSON**:公共上下文只列出已启用准则的 `id`、标题、摘要和内容版本，不再重复注入完整 Markdown。平台把完整准则原子写入 `projects/<id>/runtime-context/guidelines.json`，通过 `MISSIONCREW_GUIDELINES_FILE` 注入路径并授权所在目录；Agent 先根据摘要判断相关性，只在任务需要时读取对应条目的 `content`。正文变化会改变公共上下文中的内容版本，因此复用中的会话也会收到上下文更新。
