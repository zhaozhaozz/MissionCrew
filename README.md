...
**聊天协作(轻量)**:在 Web 聊天框里 `@角色` 布置工作;人类消息不 `@` 任何角色时默认交给项目主控调度(Agent 回复不适用——不 `@` 人正是级联的终点)。每个角色固定使用一个 runtime/model;角色定位、能力与偏好用于人类或 `@lead` 选人,不参与执行时路由。Agent 的回复原样发布到频道;回复中 `@其他角色` 即发起协作,平台自动级联触发。**所有消息(包括 Agent 之间的)对人类完全可见**。
