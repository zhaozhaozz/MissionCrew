...
- **版本化文档库**:`.missioncrew/docs/` 是所有 Runtime 都能直接读写的项目文档入口，路径同时通过兼容环境变量 `MISSIONCREW_DOCUMENTS_DIR` 注入。实际文档工作树和独立 Git 历史由平台管理，Agent 可直接创建/编辑，聊天或任务执行后平台自动提交变化。准则与 Skill 使用普通相对 Markdown 链接关联其中的文件；Web/API 仍可创建、删除、查看历史和回读旧版本。
