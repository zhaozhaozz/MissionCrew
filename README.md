...
每个角色固定使用 runtime/model/effort；角色定位、能力与偏好只供人类和 `@lead` 选人，不参与执行时路由。只有项目主控能调度其他角色，且派发和其他平台写操作一样是**显式命令**：执行 Agent Tool 的 `message.publish` 并把角色 id 写进 `mentions` 数组，平台据此生成带合法样式的可见提及并触发执行。Agent 消息正文（包括最终回复）里的任何 `@角色ID`、`@[角色ID]` 都只是普通文字，永不触发——描述已完成的派发、引用其他角色的报告都安全。执行角色看不到其他角色名册，也无权调用 `message.publish`。主控派发的角色完成或失败后，平台自动把完整结果交回主控；人类只选择一个角色时，该角色直接执行且结果留在频道，不自动触发主控；人类选择多个角色时，这些角色不会直接执行，主控根据触发消息保留的完整名单决定并行、顺序或调整人选。

```text
你（从选择器选择 dev）: @dev 只检查这个问题，结果留在频道。
你（选择 dev 和 reviewer）: @dev 修改文件；@reviewer 独立检查。  # 只启动主控协调两者
@lead 执行显式命令:         message.publish {mentions:["dev"], content:"请修改文件；验收标准：新增一行且原内容不变。"}
频道显示:                  @dev\n\n请修改文件；验收标准：新增一行且原内容不变。
@dev:                     已完成，原报告由 @reviewer 提供。  # 正文 @reviewer 仅是引用
@lead 最终回复:             已派发 @reviewer 独立检查。       # 总结里写任何 @ 都不会再触发
```

- **提及可信边界**:Web 只接受选择器生成并由后端重新校验的 Unicode 字符范围；粘贴或手动输入的 `@xxx` 不会升级为合法提及。Agent 侧唯一的派发通道是 `message.publish` 的 `mentions` 参数（仅主控可用，未知角色和主控自身会被拒绝），返回值会注明实际启动与因链预算未启动的角色；Agent 正文里的 `@`、`@[ ]` 一律不解析,主控正文若残留 `@[已知角色]` 旧语法,平台会补一条"不再触发"的提示。例外:无选择器的人类 CLI(`mc chat send`)保留 `@[角色ID]` 作为人类手动的显式派发语法。消息持久化用户选择的角色名单与合法提及范围；人类多选时实际执行目标单独收敛为主控，主控仍能从触发 JSON 读取原始名单。
- **顺序协作**:未来步骤直接写普通 `@reviewer` 作为说明即可，不会提前触发；前序结果回到主控后，主控再执行一次 `message.publish` 明确发起下一步。
...
- **防失控**:不限制主控的调度层级；只保留单条协作链的 Agent 执行总次数兜底，项目级可配置、默认 100 次。主控在预算内自主拆解和协作，达到上限时平台会在频道里说明；`message.publish` 拒绝把主控自己写进 `mentions`。
...
聊天执行并发上限默认 16(全部项目/频道共享的执行线程池),可用 `mc serve --chat-workers <N>` 或环境变量 `MISSIONCREW_CHAT_MAX_WORKERS` 调整;同频道同角色因复用 Runtime 会话始终串行,不受该值影响。
