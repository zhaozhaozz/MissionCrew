...
**聊天协作(轻量)**:在 Web 聊天框里提交需求;人类消息不 `@` 任何角色时默认交给项目主控调度，也可直接 `@角色`。每个角色固定使用 runtime/model/effort;角色定位、能力与偏好只供人类和 `@lead` 选人,不参与执行时路由。**只有项目主控能在 Agent 回复中 `@其他角色` 发起工作**;执行角色看不到其他角色名册和历史，完成或失败后由平台把完整结果自动交回主控。所有调度与执行结果都对人类可见，消息头显示实际执行组合。
...
**调度模型**:角色定位(人格)是“选人”的专长画像，**不是任务描述**。主控根据需求与项目章程对照私有名册拆解分派，为每个执行角色写清背景、要求和验收标准；执行角色只收到当前任务简报、项目上下文和准则，不会拿到其他执行角色的名册或对话。平台把结果完整交回主控，由主控核验并决定下一步。
