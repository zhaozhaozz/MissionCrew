...
- **运行状态**:查看全局 Runtime 实例、调用历史，以及本机已登录 Codex、Claude、Kimi、Grok 账户的限额窗口和重置时间；可选择开启角色联动，在额度耗尽时自动停用相关角色，并在重置到点后恢复。
...
**角色启停**:人类可在项目设置中临时停用角色。停用角色不会进入 Agent 名册、聊天提及选择器或新的派发,对 Agent 而言与角色不存在相同;角色配置、聊天历史与原生会话仍会保留,已经开始的 turn 也不会中断,重新启用后即可继续使用。当前项目主控不能直接人工停用,需要先把主控切换到另一个已启用角色。运行状态页可另行开启全局「角色联动」：任务结束或页面刷新发现某个账户窗口达到 100% 时，平台自动停用固定使用该 Runtime 的角色，已知重置时间到点后只通过本地计时恢复，不进行周期性用量探测；Claude Fable 使用独立周窗口。
