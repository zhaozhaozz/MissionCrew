...
首次初始化的全局模板包含:`@lead` 主管(调度,不亲自实现)、`@dev` 开发、`@reviewer` 评审、`@expert` 专家(深度攻坚)、`@vision` 视觉验证(多模态)、`@secure` 安全审查、`@tester` 测试、`@scribe` 文档。Web「全局设置」可维护新项目角色模板和 Runtime；各项目设置可独立增删改已复制的角色，并管理频道、项目准则与验证规则。YAML 方式见 `examples/roles.yaml`。
