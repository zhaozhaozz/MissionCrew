...
- **后端是全局资源**:本机安装的 Agent CLI 由所有项目共享(如同 Multica 的 Runtime),路由与配额全局记账。
- Web 顶部的项目切换器是所有页面的第一入口;CLI 用 `-p/--project` 限定。

## 两种协作面

**聊天协作(轻量)**:在 Web 聊天框里 `@角色` 布置工作。角色是"人格 + 领域上下文 + 路由约束",实际执行后端由路由器按能力/成本/安全动态选择。Agent 的回复原样发布到频道;回复中 `@其他角色` 即发起协作,平台自动级联触发。**所有消息(包括 Agent 之间的)对人类完全可见**。
...
角色 = **角色定位(人格)+ 结构化配置**,两者严格分离(参考 Multica 的 instructions/model 分离):

- **角色定位**(description):自由文本,平台原样装配、不改写。它的用途是**供调度方选人**(名册中展示给人类和其他角色),在执行时也会告知角色自身,但明确标注"不是任务,不要据此自行发挥"——任务只来自 @ 消息里的简报。
- **偏好标签**(traits):`快速`、`低成本`、`高质量`、`深度攻坚`、`多模态`、`联网检索`、`代码评审`、`安全审查`、`适合测试`、`适合文档`。偏好只影响两件事:路由(档位窗口、能力要求)和角色名册展示(让其他 Agent 知道该找谁协作)。
- **执行方式**二选一:
  - **自动路由**(默认):按偏好 + 能力要求 + 档位窗口,由路由器动态选后端;
  - **固定组合**:`pinned_backend` + `pinned_model` 直接指定 Agent 与模型(如 CI 管家固定 `claude-eco`/`haiku`),跳过路由。
...
| **Role(角色)** | 项目内可 @ 的身份:人格(选人画像)+ 路由约束/固定组合,项目之间互不可见 |
...
```text
     ...
     │  路由: 安全→能力→适配→成功率→成本(router.py,决策可审计)
     ...
     │  claude / codex / opencode / copilot / cursor-agent / …
     │  每频道/每任务隔离工作区
     └──────────────────────────────┘
```
