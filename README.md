...
- **运行状态**:查看全局 Runtime 实例、调用历史，以及本机已登录 Codex、Claude、Kimi、Grok 账户的限额窗口和重置时间。
...
**角色启停**:人类可在项目设置中临时停用角色。停用角色不会进入 Agent 名册、聊天提及选择器或新的派发,对 Agent 而言与角色不存在相同;角色配置、聊天历史与原生会话仍会保留,已经开始的 turn 也不会中断,重新启用后即可继续使用。当前项目主控不能直接停用,需要先把主控切换到另一个已启用角色。
