...
// 数据目录与服务默认一致:~/.missioncrew,已导出的 MISSIONCREW_HOME 优先;
// pm2 日志跟数据放在一起,仓库内不落任何运行时文件。
const mcHome =
  process.env.MISSIONCREW_HOME || path.join(os.homedir(), ".missioncrew");
  ...
  apps: [
    {
      ...
      // 端口被旧实例占用时的崩溃重试间隔
      restart_delay: 2000,
      out_file: path.join(mcHome, "server.log"),
      error_file: path.join(mcHome, "server.log"),
    },
  ],
};
